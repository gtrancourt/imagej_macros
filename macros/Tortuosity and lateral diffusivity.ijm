...
//   (instead of iterating over multiple zip files)
  ...
  setBatchMode(false); //newly opened windows are not displayed -- up to 20x faster
...
run("Geodesic Distance Map 3D", "marker="+stomataTitle+" mask="+airspaceTitle+" distances=[Svensson <3,4,5,7>] normalize");
saveAs("tiff", outputDir + name + "-L_geo");
Lgeo = getTitle();
LgeoID = getImageID();

// Create the Epidermis distance map
run("Geodesic Distance Map 3D", "marker="+epidermisTitle+" mask="+mesophylTitle+" distances=[Svensson <3,4,5,7>] normalize");
